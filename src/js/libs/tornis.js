...
class Tornis {
  ...
  constructor() {
    ...
    this.body = document.querySelector('#scroller')
    // device orientation
    this.lastAlpha = 0;
    this.lastBeta = 0;
    this.lastGamma = 0;
  ...
  */
  ...
  update() {
    ...
    this.yOffset = -this.body.getBoundingClientRect().y
    this.xOffset = -this.body.getBoundingClientRect().x
    ...
    if (this.xOffset == this.lastX && this.scrollXVelocity != 0) {
      this.scrollXVelocity = 0;
      this.scrollChange = true;
    }

    // reset scroll Y velocity
    if (this.yOffset == this.lastY && this.scrollYVelocity != 0) {
      this.scrollYVelocity = 0;
      this.scrollChange = true;
    }
    
    // check scroll X
    if (this.xOffset != this.lastX) {
      this.scrollChange = true;
      this.scrollXVelocity = Math.floor(this.xOffset - this.lastX);
      this.lastX = this.xOffset;
    }
    
    // check scroll Y
    if (this.yOffset != this.lastY) {
      this.scrollChange = true;
      this.scrollYVelocity = Math.floor(this.yOffset - this.lastY);
      this.lastY = this.yOffset;
    }
